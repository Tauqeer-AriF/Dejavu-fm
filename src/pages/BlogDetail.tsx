...
const parseContent = (content: string): ContentPart[] => {
  ...
  while ((match = imagePattern.exec(content)) !== null) {
    ...
    parts.push({ type: "image", caption: match[1] || "Blog image", url: match[2] });
...
export default function BlogDetail() {
  ...
  if (isError || !post) {
    return (
      <div className="py-24 text-center space-y-6">
        ...
        <Link to="/blog" className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-white text-dark-bg text-[10px] font-black uppercase tracking-widest">
  ...
  return (
    ...
    >
      <Link to="/blog" className="inline-flex items-center gap-2 text-white/40 hover:text-white transition-colors text-[10px] font-black uppercase tracking-[0.25em]">
        <ArrowLeft className="w-4 h-4" />
        Back to Blog
      </Link>
