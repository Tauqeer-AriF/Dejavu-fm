...
export function AdminEmail() {
  ...
  const applyPreset = (preset: 'brevo' | 'resend' | 'webmail-ssl' | 'webmail-tls' | 'gmail-ssl' | 'gmail-tls' | 'outlook') => {
    if (preset === 'brevo') {
      setHost('smtp-relay.brevo.com');
      setPort('587');
      setSecure(false);
      toast.info("Brevo (Sendinblue) Cloud SMTP preset loaded. Ideal for Cloud and Server hosting! (300 free emails/day)");
    } else if (preset === 'resend') {
      setHost('smtp.resend.com');
      setPort('465');
      setSecure(true);
      toast.info("Resend Cloud SMTP preset loaded.");
    } else if (preset === 'webmail-ssl') {
      setHost('mail.dejavufm.com');
      setPort('465');
      setSecure(true);
      toast.info("Custom Webmail SSL (Port 465) preset loaded. Use your domain email and password.");
    } else if (preset === 'webmail-tls') {
      setHost('mail.dejavufm.com');
      setPort('587');
      setSecure(false);
      toast.info("Custom Webmail TLS (Port 587) preset loaded. Use your domain email and password.");
    } else if (preset === 'gmail-ssl') {
      setHost('smtp.gmail.com');
      setPort('465');
      setSecure(true);
      toast.info("Gmail SSL (Port 465) preset loaded. Note: Requires Google App Password.");
    } else if (preset === 'gmail-tls') {
      setHost('smtp.gmail.com');
      setPort('587');
      setSecure(false);
      toast.info("Gmail TLS (Port 587) preset loaded. Note: Requires Google App Password.");
  ...
  const handleRefreshAll = async () => {
    ...
    try {
      ...
      toast.success("Email Suite Synchronised", {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
          <div className={`lg:col-span-2 p-4 sm:p-6 rounded-3xl border ${isLightMode ? 'bg-white border-slate-200 shadow-sm' : 'bg-white/5 border-white/10'}`}>
            <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 mb-5 sm:mb-6 pb-4 border-b ${isLightMode ? 'border-slate-200' : 'border-white/10'}`}>
              ...
              {/* Professional Quick Presets Dropdown */}
              <div className="relative" ref={presetDropdownRef}>
                <button
                  ...
                  className={`flex items-center gap-2 px-3.5 py-2 rounded-xl border text-xs font-bold transition-all shadow-sm ${
                    isPresetDropdownOpen
                      ? isLightMode
                        ? 'bg-purple-50 border-purple-300 text-purple-700 ring-2 ring-purple-400/20'
                        : 'bg-purple-950/40 border-purple-500/50 text-purple-300 ring-2 ring-purple-500/20'
                      : isLightMode
                        ? 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50 hover:border-slate-400'
                        : 'bg-black/40 border-white/15 text-slate-200 hover:bg-white/10 hover:border-white/25'
                  }`}
                  aria-expanded={isPresetDropdownOpen}
                >
                  <Sparkles className="w-3.5 h-3.5 text-purple-500 shrink-0" />
                  <span>Quick Presets</span>
                  <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isPresetDropdownOpen ? 'rotate-180 text-purple-500' : 'text-slate-400'}`} />
                </button>
                ...
                <AnimatePresence>
                  {isPresetDropdownOpen && (
                    <motion.div
                      ...
                      className={`absolute right-0 top-full mt-2 w-80 sm:w-96 rounded-2xl shadow-2xl border z-40 overflow-hidden backdrop-blur-xl ${
                        isLightMode ? 'bg-white/95 border-slate-200 text-slate-900 shadow-purple-900/10' : 'bg-slate-950/95 border-white/15 text-white shadow-black/80'
                      }`}
                    >
                      <div className={`p-3 border-b ${isLightMode ? 'bg-slate-50/80 border-slate-200' : 'bg-white/5 border-white/10'}`}>
                        <div className="flex items-center justify-between">
                          <span className={`text-[11px] font-bold uppercase tracking-wider ${isLightMode ? 'text-slate-700' : 'text-slate-300'}`}>
                            Select SMTP Provider
                          </span>
                          <span className="text-[10px] px-2 py-0.5 rounded-full font-bold bg-purple-500/15 text-purple-600 dark:text-purple-300">
                            Auto-Configure
                          </span>
                        </div>
                        <p className={`text-[11px] mt-0.5 ${isLightMode ? 'text-slate-500' : 'text-slate-400'}`}>
                          Auto-populates optimal host, port & encryption settings.
                        </p>
                      </div>

                      <div className="p-2 space-y-1 max-h-[380px] overflow-y-auto">
                        {/* Section 1: Cloud & Server Best */}
                        <div className={`px-2 pt-1.5 pb-1 text-[10px] font-bold uppercase tracking-wider ${isLightMode ? 'text-amber-700' : 'text-amber-400'}`}>
                        ...
                        <button
                          ...
                          className={`w-full text-left p-2.5 rounded-xl transition-all flex items-start justify-between gap-2 ${
                            host === 'smtp-relay.brevo.com'
                              ? isLightMode ? 'bg-amber-50 border border-amber-200' : 'bg-amber-500/15 border border-amber-500/30'
                              : isLightMode ? 'hover:bg-slate-100' : 'hover:bg-white/5'
                          }`}
                        >
                          <div>
                            <div className="flex items-center gap-1.5">
                              <span className="text-xs font-bold">Brevo (Sendinblue)</span>
                        ...
                        <button
                          ...
                          className={`w-full text-left p-2.5 rounded-xl transition-all flex items-start justify-between gap-2 ${
                            host === 'smtp.resend.com'
                              ? isLightMode ? 'bg-purple-50 border border-purple-200' : 'bg-purple-500/15 border border-purple-500/30'
                              : isLightMode ? 'hover:bg-slate-100' : 'hover:bg-white/5'
                          }`}
                        >
                          <div>
                            <div className="flex items-center gap-1.5">
                              <span className="text-xs font-bold">Resend Cloud SMTP</span>
                              <span className="text-[10px] px-1.5 py-0.2 rounded font-bold bg-slate-500/20 text-slate-700 dark:text-slate-300">
                                Cloud API
                              </span>
                            </div>
                            <p className={`text-[11px] mt-0.5 ${isLightMode ? 'text-slate-500' : 'text-slate-400'}`}>
                        ...
                        <div className={`px-2 pt-2.5 pb-1 text-[10px] font-bold uppercase tracking-wider ${isLightMode ? 'text-indigo-700' : 'text-indigo-400'}`}>
                          🌐 Custom Domain / Webmail
                        </div>
                        <button
                          type="button"
                          onClick={() => {
                            applyPreset('webmail-ssl');
                            setIsPresetDropdownOpen(false);
                          }}
                          className={`w-full text-left p-2.5 rounded-xl transition-all flex items-start justify-between gap-2 ${
                            host === 'mail.dejavufm.com' && port === '465'
                              ? isLightMode ? 'bg-indigo-50 border border-indigo-200' : 'bg-indigo-500/15 border border-indigo-500/30'
                              : isLightMode ? 'hover:bg-slate-100' : 'hover:bg-white/5'
                          }`}
                        >
                          <div>
                            <div className="flex items-center gap-1.5">
                              <span className="text-xs font-bold">Webmail (Direct SSL)</span>
                              <span className="text-[10px] px-1.5 py-0.2 rounded font-bold bg-indigo-500/20 text-indigo-700 dark:text-indigo-300">
                                cPanel / Hostinger
                              </span>
                            </div>
                            <p className={`text-[11px] mt-0.5 ${isLightMode ? 'text-slate-500' : 'text-slate-400'}`}>
                              mail.dejavufm.com • Port 465 (SSL)
                            </p>
                          </div>
                          {host === 'mail.dejavufm.com' && port === '465' && (
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            applyPreset('webmail-tls');
                            setIsPresetDropdownOpen(false);
                          }}
                          className={`w-full text-left p-2.5 rounded-xl transition-all flex items-start justify-between gap-2 ${
                            host === 'mail.dejavufm.com' && port === '587'
                              ? isLightMode ? 'bg-indigo-50 border border-indigo-200' : 'bg-indigo-500/15 border border-indigo-500/30'
                              : isLightMode ? 'hover:bg-slate-100' : 'hover:bg-white/5'
                          }`}
                        >
                          <div>
                            <div className="flex items-center gap-1.5">
                              <span className="text-xs font-bold">Webmail (STARTTLS)</span>
                              <span className="text-[10px] px-1.5 py-0.2 rounded font-bold bg-cyan-500/20 text-cyan-700 dark:text-cyan-300">
                                Port 587
                              </span>
                            </div>
                            <p className={`text-[11px] mt-0.5 ${isLightMode ? 'text-slate-500' : 'text-slate-400'}`}>
                              mail.dejavufm.com • Port 587 (TLS)
                            </p>
                          </div>
                          {host === 'mail.dejavufm.com' && port === '587' && (
                        ...
                        {/* Section 3: Consumer / Enterprise */}
                        <div className={`px-2 pt-2.5 pb-1 text-[10px] font-bold uppercase tracking-wider ${isLightMode ? 'text-slate-600' : 'text-slate-400'}`}>
                          ✉️ Consumer & Enterprise
                        </div>
                        <button
                          type="button"
                          onClick={() => {
                            applyPreset('gmail-ssl');
                            setIsPresetDropdownOpen(false);
                          }}
                          className={`w-full text-left p-2.5 rounded-xl transition-all flex items-start justify-between gap-2 ${
                            host === 'smtp.gmail.com' && port === '465'
                              ? isLightMode ? 'bg-emerald-50 border border-emerald-200' : 'bg-emerald-500/15 border border-emerald-500/30'
                              : isLightMode ? 'hover:bg-slate-100' : 'hover:bg-white/5'
                          }`}
                        >
                          <div>
                            <div className="flex items-center gap-1.5">
                              <span className="text-xs font-bold">Google Gmail (SSL 465)</span>
                              <span className="text-[10px] px-1.5 py-0.2 rounded font-bold bg-emerald-500/20 text-emerald-700 dark:text-emerald-300">
                                App Password
                              </span>
                            </div>
                            <p className={`text-[11px] mt-0.5 ${isLightMode ? 'text-slate-500' : 'text-slate-400'}`}>
                        ...
                        >
                          <div>
                            <div className="flex items-center gap-1.5">
                              <span className="text-xs font-bold">Google Gmail (TLS 587)</span>
                              <span className="text-[10px] px-1.5 py-0.2 rounded font-bold bg-purple-500/20 text-purple-700 dark:text-purple-300">
                                App Password
                              </span>
                            </div>
                            <p className={`text-[11px] mt-0.5 ${isLightMode ? 'text-slate-500' : 'text-slate-400'}`}>
                        ...
                        <button
                          ...
                          className={`w-full text-left p-2.5 rounded-xl transition-all flex items-start justify-between gap-2 ${
                            host === 'smtp.office365.com'
                              ? isLightMode ? 'bg-blue-50 border border-blue-200' : 'bg-blue-500/15 border border-blue-500/30'
                              : isLightMode ? 'hover:bg-slate-100' : 'hover:bg-white/5'
                          }`}
                        >
                          <div>
                            <div className="flex items-center gap-1.5">
                              <span className="text-xs font-bold">Microsoft Outlook / Office 365</span>
                              <span className="text-[10px] px-1.5 py-0.2 rounded font-bold bg-blue-500/20 text-blue-700 dark:text-blue-300">
                                Office 365
                              </span>
                            </div>
                            <p className={`text-[11px] mt-0.5 ${isLightMode ? 'text-slate-500' : 'text-slate-400'}`}>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="••••••••••••••••"
                      required
                      className={`w-full pl-3.5 pr-10 sm:pl-4 sm:pr-10 py-2 sm:py-2.5 rounded-xl border font-mono text-xs sm:text-sm ${
                        isLightMode ? 'bg-slate-50 border-slate-300 text-slate-900 placeholder:text-slate-400' : 'bg-black/30 border-white/10 text-white placeholder:text-slate-500'
                      }`}
                    />
                    <button
                      ...
                      className={`absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded-lg transition-colors ${
                        isLightMode ? 'text-slate-500 hover:text-slate-800 hover:bg-slate-200/50' : 'text-slate-400 hover:text-white hover:bg-white/10'
                      }`}
          ...
          <div className="space-y-4 sm:space-y-6">
            <div className={`p-4 sm:p-6 rounded-3xl border ${isLightMode ? 'bg-amber-50/80 border-amber-300' : 'bg-amber-950/20 border-amber-800/40'}`}>
              <h3 className={`text-sm sm:text-base font-bold flex items-center gap-2 mb-2 ${isLightMode ? 'text-amber-950' : 'text-amber-300'}`}>
                <Sparkles className="w-4 h-4 text-amber-600 shrink-0" />
                Option 1: Brevo Cloud SMTP (Recommended for Cloud Hosting)
              </h3>
              <p className={`text-[11px] mb-3 ${isLightMode ? 'text-amber-900' : 'text-amber-200'}`}>
                Modern cloud hosting platforms frequently restrict direct consumer Gmail connections to prevent spam. Brevo offers <strong>300 free emails/day</strong> and connects reliably without socket drops.
              </p>
              <ol className={`text-xs space-y-2 list-decimal pl-4 ${isLightMode ? 'text-slate-800' : 'text-slate-300'}`}>
                <li>Create a free account on <a href="https://www.brevo.com" target="_blank" rel="noreferrer" className="underline font-bold text-amber-600 dark:text-amber-400">brevo.com</a>.</li>
                <li>Go to <strong>Transactional</strong> → <strong>Settings</strong> → <strong>Configuration</strong> (SMTP).</li>
                <li>Click the <strong className="text-amber-700 dark:text-amber-400">Brevo (Sendinblue)</strong> preset above, paste your Brevo login email and SMTP Master Key, then save.</li>
              </ol>
            </div>

            <div className={`p-4 sm:p-6 rounded-3xl border shadow-sm ${isLightMode ? 'bg-indigo-50/70 border-indigo-200' : 'bg-indigo-950/20 border-indigo-800/40'}`}>
              <h3 className={`text-xs sm:text-sm font-bold flex items-center gap-2 mb-2 ${isLightMode ? 'text-indigo-950' : 'text-indigo-300'}`}>
                <Server className="w-4 h-4 text-indigo-500 shrink-0" />
                Option 2: Custom Domain Webmail / cPanel
              </h3>
              <p className={`text-[11px] mb-3 ${isLightMode ? 'text-indigo-900' : 'text-indigo-200'}`}>
                If you have an email inbox with your radio domain (e.g. Hostinger, cPanel, Namecheap, Google Workspace):
              </p>
              <ul className={`text-xs font-mono space-y-1.5 p-3 rounded-2xl border break-all ${
                isLightMode ? 'bg-white border-indigo-200 text-slate-800' : 'bg-black/40 border-white/10 text-slate-200'
              }`}>
                <div><strong className={isLightMode ? 'text-slate-900 font-bold' : 'text-white'}>Host:</strong> mail.dejavufm.com (or your mail server)</div>
                <div><strong className={isLightMode ? 'text-slate-900 font-bold' : 'text-white'}>Port:</strong> 465 (SSL checked) or 587</div>
                <div><strong className={isLightMode ? 'text-slate-900 font-bold' : 'text-white'}>User:</strong> studio@dejavufm.com</div>
                <div><strong className={isLightMode ? 'text-slate-900 font-bold' : 'text-white'}>Password:</strong> Your webmail email password</div>
              </ul>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4 sm:space-y-5 max-w-3xl">
            ...
            <div>
              ...
              <input
                ...
                placeholder="🎉 Big Friday Night Show Line-up!"
                required
                className={`w-full px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl border text-xs sm:text-sm ${
                  isLightMode ? 'bg-slate-50 border-slate-300 text-slate-900 placeholder:text-slate-400' : 'bg-black/30 border-white/10 text-white placeholder:text-slate-500'
                }`}
                ...
                <div className="space-y-4">
                  ...
                  {testResult && (
                    ...
                    >
                      ...
                      <div className="font-mono text-[11px] break-words mb-2">{testResult.message || testResult.error}</div>
                      
                      {!testResult.success && (testResult.error?.includes('ETIMEDOUT') || testResult.error?.includes('timeout') || testResult.error?.includes('CONN')) && (
                        <div className={`mt-2.5 pt-2.5 border-t ${isLightMode ? 'border-rose-200 text-slate-800' : 'border-rose-500/20 text-slate-200'}`}>
                          <p className="text-[11px] font-bold mb-1.5 text-amber-700 dark:text-amber-300">💡 Cloud & Server Hosting Guidance:</p>
                          <p className="text-[11px] mb-2 leading-relaxed">Direct consumer Gmail connections are frequently restricted on cloud servers to prevent abuse. Switch to Brevo (300 free/day) or your custom domain webmail:</p>
                          <div className="flex flex-wrap gap-1.5">
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-amber-600 hover:bg-amber-700 text-white shadow-sm"
                            >
                              Load Brevo Preset ⭐
                            </button>
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm"
                            >
                              Load Webmail Preset
                            </button>
                          </div>
