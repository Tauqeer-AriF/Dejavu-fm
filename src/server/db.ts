...
export function initDb() {
  ...
  try {
    ...
    CREATE TABLE IF NOT EXISTS djs (
      ...
      mixcloud TEXT,
      facebook TEXT,
      badge1 TEXT DEFAULT 'Resident',
      badge2 TEXT DEFAULT 'Underground'
    );
    ...
    CREATE TABLE IF NOT EXISTS arch421_registrations (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      name TEXT NOT NULL,
      email TEXT NOT NULL,
      status TEXT DEFAULT 'pending',
  ...
  runMigration('dj_badge_fields_v1', "ALTER TABLE djs ADD COLUMN badge1 TEXT DEFAULT 'Resident'; ALTER TABLE djs ADD COLUMN badge2 TEXT DEFAULT 'Underground';");
  ...
  if (countSettings.count === 0) {
    ...
    db.prepare('INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT DO NOTHING').run('feat_accessibility', '1');
    db.prepare('INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT DO NOTHING').run('feat_studio', '1');
    db.prepare('INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT DO NOTHING').run('feat_meta', '1');
    db.prepare('INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT DO NOTHING').run('feat_backup', '1');
    db.prepare('INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT DO NOTHING').run('feat_email', '1');
